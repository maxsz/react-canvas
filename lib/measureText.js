...
var loadHypher = function (locale) {
  if (typeof(locale) !== 'object') {
    return
  }

  if (hypher != undefined && hypherLocale === locale) {
    return hypher
  }

  hypherLocale = locale;
  hypher = new Hypher(hypherLocale);
...
var getHyphenationOpportunities = function(text, hyphens, locale) {
  var memoKey = hyphens + locale + text;
...
var getParagraphMetrics = function(text, hyphens, locale, fontFace, fontSize) {
  var memoKey = text + hyphens + locale + fontFace.id + fontSize;
  var memoized = _paragraphMetricsMemos[memoKey];
  if (memoized) { return memoized; }

  var metrics = {};
  metrics.fragments = getHyphenationOpportunities(text, hyphens, locale);
...
// var _lineBreakMemos = {};
module.exports = function measureText(width, text, hyphens, locale, fontFace, fontSize, lineHeight, breakingStrategy) {
  ...
  var metrics = getParagraphMetrics(text, hyphens, locale, fontFace, fontSize);
  var measuredSize = {};
